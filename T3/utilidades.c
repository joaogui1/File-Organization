...
void atribute_regs(registro *a, registro *b){
  ...
  // b->tamNomeServidor = (a->tamNomeServidor > 0) ? a->tamNomeServidor:0;
  ...
  // b->tamCargoServidor = (a->tamCargoServidor > 0) ? a->tamCargoServidor : 0;
...
int write_to_page(char **page, int sz_pg, registro ant_reg){
  ...
  if(ant_reg.tamNomeServidor > 0){
    // ant_reg.tamNomeServidor += 1; //tamanho da tag
...
/*
Funcao que escreve um registro em uma página de disco
e se não couber na página, preenche o fim dela com @ e
escreve a página no arquivo binário
input:  output: arquivo binário onde a página será escrita
        page, ponteiro para array de chars que representa
          nossa página de disco
        sz_pg, posicao que estamos na página antes da
          funcao comecar a executar
        registro ant_reg
        registro reg: registro que vamos gravar na
          página de disco
output: sz_pg, posicao que estamos na página após a
          execucao da funcao
*/
int new_write_to_page(FILE *output, char **page, int sz_pg, registro ant_reg, registro reg) {
  ...
  if (sz_pg + reg.tamRegistro + 5 > 32000){
    ant_reg.tamRegistro += (32000 - sz_pg);
    
    int offset = 14 + 8 + 4 + 8 + 4; //volta o ponteiro para a posicao onde ele escreveu o tamanho do registro anterior
    if (ant_reg.tamNomeServidor > 0)  offset +=  4 + ant_reg.tamNomeServidor;
    if (ant_reg.tamCargoServidor > 0) offset += 4 + ant_reg.tamCargoServidor;
    
    memcpy(*page + sz_pg - offset, &ant_reg.tamRegistro, sizeof(ant_reg.tamRegistro));
    fwrite(*page, sizeof(char), 32000, output);
    memset(*page, '@', 32000);
    sz_pg = 0;
  }

  return write_to_page(page, sz_pg, reg);
}
...
/*
Lê um registro direto de um arquivo
input: FILE *input: arquivo binário 
       registro *reg, registro de dados que vai guardar o que foi lido
output: void
*/
void read_register_from_binary(FILE *input, registro *reg) {
  ...
  fread(&(reg->tamRegistro), sizeof(int), 1, input);

  fread(&(reg->encadeamentoLista), sizeof(long), 1, input);
  ...
  if(reg->tamRegistro > 34){
    fread(&(aux_size), sizeof(int), 1, input);
    fread(&(aux_tag), sizeof(char), 1, input);
    

    if(aux_tag == 'n'){
      reg->tamNomeServidor = aux_size;
      fread(reg->nomeServidor, sizeof(char), reg->tamNomeServidor - 1, input);

      fread(&(aux_size), sizeof(int), 1, input);
      fread(&(aux_tag), sizeof(char), 1, input);

      if (aux_tag == 'c') {
        reg->tamCargoServidor = aux_size;
        fread(reg->cargoServidor, sizeof(char), reg->tamCargoServidor - 1, input);
      }
      else{
      ...
      fseek(input, reg->tamRegistro - 39, SEEK_CUR);
    }
  }
